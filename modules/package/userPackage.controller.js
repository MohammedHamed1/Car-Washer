...
exports.createUserPackage = async (req, res) => {
  try {
    const { packageId, userId, washesLeft, expiry } = req.body;
    
    const userPackage = new UserPackage({
      user: userId || req.user._id,
      package: packageId,
      washesLeft: washesLeft || 0,
      expiry: expiry || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000), // 30 days
      status: 'active'
    });
    
    await userPackage.save();
    
    const populatedUserPackage = await UserPackage.findById(userPackage._id)
      .populate('package')
      .populate('user');
    
    res.status(201).json(populatedUserPackage);
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
};
